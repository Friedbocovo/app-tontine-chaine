...
export default function KYC() {
  ...
  // ✅ FIX : useEffect pour attacher le stream au video element
  // après que React ait rendu le composant
  useEffect(() => {
  ...
  const startCamera = async () => {
    ...
    try {
      ...
      // ✅ FIX : On met cameraActive à true d'abord
      // pour que React rende le <video>, puis useEffect attache le stream
      setCameraActive(true);

      // Simuler détection visage après 2s
      ...
      if (err.name === "NotAllowedError" || err.name === "PermissionDeniedError") {
        ...
        setCameraError("Impossible d'accéder à la caméra. Réessayez.");
      }
    }
  };
  ...
  // Nettoyage quand on quitte le composant
  useEffect(() => {
    return () => {
      stopCamera();
    };
  ...
  const capturePhoto = () => {
    ...
    // Retourner horizontalement pour effet miroir naturel
  ...
  const handleSubmit = () => {
    ...
    if (step === 2) {
      if (!selfieCapture) { setError("Veuillez prendre votre selfie avec la caméra"); return; }
      setLoading(true);
      setTimeout(() => {
        setLoading(false);
        ...
        <div className="w-full max-w-md text-center">
          ...
          <div className="flex flex-col gap-3 mb-10 text-left">
            {[
              ...
              { label: "Vérification automatique en cours", done: false, loading: true },
          ...
          <div className="bg-blue-50 rounded-2xl p-4 mb-8 text-left">
            <div className="flex items-start gap-3">
              ...
              <p className="text-xs text-blue-600 leading-relaxed">
                Vos documents ne sont jamais stockés sur nos serveurs.
                Seule leur empreinte cryptographique est enregistrée sur la blockchain.
              </p>
            </div>
          </div>
          <button
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white">
      ...
      {/* Header */}
      <div className="flex items-center justify-between px-6 pt-8 pb-4 w-full max-w-2xl mx-auto">
        <button
        ...
        <span className="text-sm font-semibold text-gray-500">Vérification d'identité (KYC)</span>
        <div className="w-9" />
      </div>

      <div className="flex-1 px-6 pb-10 overflow-y-auto w-full max-w-2xl mx-auto">
        ...
        {step === 1 && (
          <div className="flex flex-col gap-4">
            {documentTypes.map((doc) => (
              <div key={doc.id}>
                <button
                  onClick={() => { setSelectedDoc(selectedDoc === doc.id ? null : doc.id); setDocumentFile(null); }}
        ...
        {step === 2 && (
          <div className="flex flex-col items-center gap-6 w-full">
            ...
            {/* ✅ FIX PRINCIPAL : le <video> est TOUJOURS rendu dans le DOM
                quand cameraActive est true, mais caché sinon.
                Le useEffect en haut attache le stream après le rendu. */}
            {cameraActive && (
              <div className="flex flex-col items-center gap-4 w-full">
                <div className="relative w-full max-w-sm mx-auto overflow-hidden rounded-3xl bg-black"
                  style={{ height: "360px" }}>

                  {/* ✅ FIX : pas de style transform sur la vidéo elle-même,
                      on utilise la propriété CSS directement */}
                  <video
                    ref={videoRef}
                    autoPlay
                    playsInline
                  ...
                  <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                    <div className={`w-44 h-52 rounded-full border-4 transition-all duration-500 ${
                      faceDetected ? "border-green-400 shadow-lg" : "border-white border-dashed opacity-70"
                    }`} />
                  </div>
                  ...
                  {countdown !== null && (
                    <div className="absolute inset-0 flex items-center justify-center bg-black bg-opacity-50 rounded-3xl">
                      <span className="text-white font-bold" style={{ fontSize: "96px" }}>
                        {countdown}
                      </span>
                    </div>
                  )}
        ...
        {!cameraActive && (
          ...
          >
            {loading ? (
              ...
              step === 1 ? "Continuer vers le selfie →" : "Soumettre et vérifier →"
            )}
          </button>
        )}
      </div>
