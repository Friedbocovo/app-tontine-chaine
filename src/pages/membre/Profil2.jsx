        ...
        <div className="flex justify-between items-center mb-3">
          <p className="text-sm font-bold text-gray-800 flex items-center gap-2">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
              <rect x="2" y="5" width="20" height="14" rx="3" stroke="#1B5E4B" strokeWidth="2"/>
              <path d="M2 10h20" stroke="#1B5E4B" strokeWidth="2"/>
            </svg>
            Mes comptes Mobile Money
          </p>
          <button className="text-xs text-green-700 font-semibold">Gérer</button>
        </div>
        <div className="flex flex-col gap-2">
          {demoProfile.comptesMobileMoney.map((compte) => (
            <div
              key={compte.id}
              className="flex items-center gap-3 p-3 rounded-xl bg-gray-50"
            >
              <span className="text-2xl">{compte.icon}</span>
              <div className="flex-1 min-w-0">
                <p className="font-semibold text-gray-900 text-sm">{compte.label}</p>
                <p className="text-gray-400 text-xs">{compte.numero}</p>
              </div>
              {compte.defaut && (
