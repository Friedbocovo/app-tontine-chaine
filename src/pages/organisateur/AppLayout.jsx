...
function BottomNav({ navigate }) {
  ...
  const tabs = [
    ...
    },
    {
      id: "paiements",
      label: "Paiements",
      path: "/organisateur/historique/1",
      icon: (
        <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
  ...
  const getActiveTab = () => {
    ...
    if (path === "/organisateur") return "accueil";
    if (path.includes("/tontine") || path.includes("/creer") || path.includes("/incidents")) return "tontine";
    if (path.includes("/historique") || path.includes("/paiements")) return "paiements";
...
function Sidebar({ navigate, nomUtilisateur, role = "Organisateur" }) {
  const location = useLocation();

  const tabs = [
    { id: "accueil", label: "Accueil", path: "/organisateur", emoji: "🏠" },
    { id: "tontine", label: "Mes Tontines", path: "/organisateur/tontine/1", emoji: "👥" },
    { id: "Historique", label: "Historique", path: "/organisateur/historique/1", emoji: "📅" },
    { id: "profil", label: "Profil", path: "/profil", emoji: "👤" },
  ];
  ...
  return (
    <div className="w-64 bg-white border-r border-gray-100 flex flex-col py-8 px-4 flex-shrink-0 min-h-screen sticky top-0">
      ...
      <nav className="flex flex-col gap-1 flex-1">
        {tabs.map((tab) => (
          <button
            key={tab.id}
            onClick={() => navigate(tab.path)}
            className={`flex items-center gap-3 px-4 py-3 rounded-2xl text-sm font-medium transition-all text-left ${activeTab === tab.id
      ...
      {/* Créer tontine */}
      <button
        onClick={() => navigate("/organisateur/creer/regles")}
  ...
  return (
    <>
      ...
      <div className="flex flex-col min-h-screen md:hidden" style={{ backgroundColor: bgColor }}>
        ...
        {showFab && (
          <button
            onClick={fabAction || (() => navigate("/organisateur/creer/regles"))}
